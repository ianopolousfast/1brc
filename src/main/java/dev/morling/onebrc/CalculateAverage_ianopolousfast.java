    ...
    public static Stat[] parseStats(long start1, long end2, MemorySegment buffer) {
        ...
        if (end2 == buffer.byteSize()) {
            ...
            while (end2 + index < buffer.byteSize()) {
                ...
                index = (int) processTemperature(index + keySize1 + 1, lineSize1 - keySize1 - 1, end, station1);
            }
        }
        ...
        while (start1 < end2) {
            ...
            start1 = processTemperature(start1 + keySize1 + 1, lineSize1 - keySize1 - 1, buffer, station1);
            if (start2 < end2) {
                Stat station2 = dedupeStation(start2, start2 + keySize2, buffer, stations);
                start1 = processTemperature(start2 + keySize2 + 1, lineSize2 - keySize2 - 1, buffer, station2);
            }
        }
        return stations;
    }
